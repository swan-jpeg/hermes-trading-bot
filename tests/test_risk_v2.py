"""Unit tests voor Risk Engine v2 — gerepareerde mechanismen.
...
def test_drawdown_no_lookahead() -> None:
    """Drawdown gebruikt alleen historische piek (geen look-ahead)."""
    e = _engine()
...
def test_drawdown_initialization() -> None:
    """Correcte initialisatie: eerste equity is de piek."""
    e = _engine()
    assert e.update_drawdown(100) == 0.0
...
def test_montecarlo_crash_probability_nonzero() -> None:
    """REPAIR: crash_probability moet > 0 zijn bij een crash-achtige verdeling."""
...
def test_montecarlo_no_lookahead() -> None:
    """MC gebruikt alleen de gegeven historische returns (geen look-ahead)."""
...
def test_adaptive_exposure_reduces_in_crash() -> None:
    """v2 verlaagt exposure bij diepe drawdown (adaptief)."""
    e = _engine()
    e.peak_equity = 100.0
...
def test_recovery_rebuilds_exposure() -> None:
    """Na herstel bouwt de engine exposure geleidelijk op (niet in cash)."""
    e = _engine()
    e.peak_equity = 100.0
    # Realistische returns met kleine ruis (niet constant -> corr != 1.0).
    ...
    # Geen drawdown, lage vol -> exposure bouwt op (boven 0.5).
